...
def get_batch(
    data: torch.Tensor, iter: int, batch_size: int, win_size: int, device: str
    ...
    if max_idx <= 0:
        return torch.zeros((batch_size, win_size), dtype=torch.long).to(
            device
    ...
    x: torch.Tensor = torch.stack([data[idx : idx + win_size] for idx in indices])
    y: torch.Tensor = torch.stack(
        [data[idx + 1 : idx + 1 + win_size] for idx in indices]
    )
    return x.to(device), y.to(device)


def get_optimal_dm(
    vocab_size: int, num_tbs: int, window_size: int, target_params: int = MAX_NUM_PARAMS
) -> int:
    ...
    # norm_f: dm
    # lm_head: dm * V (tied with token_emb, so V*dm is counted once)
...
def train_on_file(
    config_dict: dict[str, Any], all_tokens: list[str], curve_id: str = ""
) -> tuple[
    float,
    ...
    if os.path.exists(checkpoint_path):
        ...
        if len(ckpt.get("history", [])) >= (config_dict["iters"] // 100):
            ...
            print(f">>> RESUMING: {run_name} from iteration {ckpt['history'][-1][0]}")
    ...
    train_data = torch.tensor(
        [tok_to_idx[w] for w in train_tokens if w in tok_to_idx], dtype=torch.long
    )
    ...
    optimizer = optim.AdamW(
        model.parameters(), lr=config_dict["lr"], betas=(0.9, 0.95), weight_decay=0.1
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Train WangGPT")
    parser.add_argument(
        "--quick", action="store_true",
        help="Quick test run: 200 iters, small data subset, ~2 min total",
    )
    ...
    if args.quick:
        # Use only the first 50k characters for a fast test run
        text_data = text_data[:50_000]
        print(">>> QUICK MODE: using first 50k chars of corpus")

    all_tokens = utils.clean_tokenization(text_data)

    lrs = [3e-4]
    batch_sizes = [32]
    iters = 200 if args.quick else 10_000
    base_config = {"layers": 7, "window_size": 128, "iters": iters}
    best_curves = {}

    # Establish random baseline
    if os.path.exists("models/random_init.pt") and not args.quick:
        ...
        dm_rand = get_optimal_dm(
            utils.VOCAB_SIZE, base_config["layers"], base_config["window_size"]
        )
        model_config = Config(
            utils.VOCAB_SIZE,
            dm_rand,
        ...
        torch.save(
            {
                ...
                "vocab": utils.UNIFIED_VOCAB,
                "history": [(0, 10.0), (base_config["iters"], 10.0)],
            },
            "models/random_init.pt",
        )
        best_curves["random_init"] = [(0, 10.0), (iters, 10.0)]

    # Grid search (quick mode only runs with_pe)
    pe_variants = [True] if args.quick else [True, False]
    for use_pe in pe_variants:
        ...
        for lr, bs in product(lrs, batch_sizes):
            config = {**base_config, "lr": lr, "batch_size": bs, "use_pe": use_pe}
    ...
    save_plot(best_curves, "Star Wars GPT Model Comparison", "training_comparison.png")
